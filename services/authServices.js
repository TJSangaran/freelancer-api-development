...
exports.signup = async ({ firstname, lastname, email, password, phone, description, tags, address, dob, isErrand, readyToWork, accountnumber, accountname, bankname }) => {
    const salt = await bcrypt.genSalt(10);
    const hashedPassword = await bcrypt.hash(password, salt);
    const profileColor = Math.floor(Math.random() * 10) + 1
    const user = await User.create({ firstname, lastname, email, password: hashedPassword, phone, description, tags, address, dob, isErrand, readyToWork, accountnumber, accountname, bankname, profileColor })
